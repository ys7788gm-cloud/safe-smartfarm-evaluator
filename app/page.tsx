...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const button = stepButtonRefs.current[activeTab];
  ...
  return (
    <main>
      ...
      <section className="workspace" id="workspace">
        <nav ref={stepNavRef} className="step-nav" aria-label="평가 단계">
          {tabs.map((tab, index) => <button ref={(node) => { stepButtonRefs.current[index] = node; }} key={tab} className={activeTab === index ? "active" : ""} onClick={() => setActiveTab(index)} aria-current={activeTab === index ? "step" : undefined}><span>{completedTabs[index] ? "✓" : index + 1}</span><b className="step-label">{tab}</b></button>)}
        </nav>
